// ================== DIAGNOSTICS61.JS - VERSÃO 6.1.1 ==================
// CADEIA PROGRESSIVA DE DIAGNÓSTICO - MÓDULO DE VALIDAÇÃO AVANÇADA
// Propósito: Este módulo adiciona uma camada de validação avançada de integridade
// e performance ao sistema, sem conflitar com outros módulos da cadeia (diagnostics53-60).
// Atualizado para se integrar perfeitamente com o Core System e Support System.
// Inclui posicionamento inteligente para não sobrepor outros painéis de diagnóstico.
...
(function() {
    ...
    const PANEL_CONFIG = {
        ...
        title: '🔬 DIAGNOSTICS61 - VALIDAÇÃO AVANÇADA v6.1.1',
        width: '520px',
        defaultPosition: { left: '250px', top: '100px' } // Posição inicial estratégica
    };
    ...
    /**
     * Verifica a integridade das variáveis globais críticas do Core System.
     */
    ...
    /**
     * Analisa a performance de carregamento dos módulos críticos.
     */
    function analyzeLoadPerformance() {
        ...
        // Verificar módulos que podem não ter sido carregados via resource (ex: inline)
    ...
    function testModuleCommunication() {
        ...
        // Teste 2: MediaSystem pode ser inicializado?
        try {
            const initResult = MediaSystem?.init?.('vendas') ? '✅' : '⚠️ (já iniciado)';
            results.push({ test: 'MediaSystem.init()', status: initResult, detail: 'OK' });
        } catch (e) {
            results.push({ test: 'MediaSystem.init()', status: '❌', detail: e.message });
        }

        // Teste 3: PdfSystem pode ser inicializado?
        try {
            PdfSystem?.init?.();
        ...
        // Teste 4: LoadingManager pode ser chamado?
        try {
        ...
        // Teste 5: FilterManager pode ser usado?
        try {
    ...
    /**
     * Valida a integridade dos dados no localStorage (chave unificada 'properties').
     */
    ...
    /**
     * Executa uma verificação rápida de possíveis "zumbis" (event listeners órfãos).
     */
    function detectOrphanedElements() {
        console.group('🧟 [DIAGNOSTICS61] Detecção de Elementos/Zumbis');
        const orphaned = [];

        // Verificar modais que podem ter ficado abertos
    ...
    // ========== FUNÇÃO PARA CRIAR O PAINEL COM POSICIONAMENTO INTELIGENTE ==========
    function createPanel() {
        ...
        // --- Posicionamento Inteligente para Evitar Conflitos ---
        let calculatedLeft = PANEL_CONFIG.defaultPosition.left;
        let calculatedTop = PANEL_CONFIG.defaultPosition.top;

        // Tenta encontrar o último painel de diagnóstico na tela
        ...
        if (existingPanels.length > 0) {
            ...
            // Posiciona 30px abaixo e 30px à direita do último painel, se couber na tela
            ...
            console.log(`[DIAGNOSTICS61] Nenhum outro painel encontrado. Usando posição padrão: ${PANEL_CONFIG.defaultPosition.left}, ${PANEL_CONFIG.defaultPosition.top}`);
        }
        // ------------------------------------------------
        ...
        panel.setAttribute('data-version', '6.1.1');
        panel.style.cssText = `
            position: fixed;
            left: ${calculatedLeft};
            top: ${calculatedTop};
        ...
        // Corpo do painel (conteúdo rolável)
        ...
        resultsArea.style.cssText = `
            ...
            min-height: 200px;
        `;
        resultsArea.innerHTML = `<div style="text-align: center; color: #00ffff80;">🔄 Clique em "Executar Validação Completa"</div>`;
        ...
        function displayResults(title, data, type = 'info') {
            ...
            if (typeof data === 'object') {
                formattedData = '<pre style="background:#00000030; padding:8px; border-radius:4px; overflow-x:auto;">' + JSON.stringify(data, null, 2) + '</pre>';
            } else {
                formattedData = `<div>${data}</div>`;
            }
            resultsEl.innerHTML = `<div style="border-left: 4px solid ${type === 'error' ? '#ff5555' : '#00ffff'}; padding-left: 10px;"><strong style="color:#00ffff;">${title}</strong><br>${formattedData}</div>`;
        }

        document.getElementById('diag61-run-all')?.addEventListener('click', async () => {
            resultsEl.innerHTML = '<div style="text-align:center; color:#00ffff;">🔄 Executando todas as validações...</div>';
            const allResults = {
        ...
        document.getElementById('diag61-clear')?.addEventListener('click', () => {
            resultsEl.innerHTML = '<div style="text-align: center; color: #00ffff80;">✅ Resultados limpos.</div>';
        });

        // ========== DRAG & DROP (REUTILIZADO) ==========
    ...
    function initialize() {
        console.log('%c🔬 [DIAGNOSTICS61] v6.1.1 - Módulo de Validação Avançada Carregado', 'color: #00ffff; font-weight: bold;');

        // Criar o painel se a URL tiver o parâmetro 'diagnostics=true'
        if (window.location.search.includes('diagnostics=true')) {
            setTimeout(() => {
                createPanel();
                console.log('✅ [DIAGNOSTICS61] Painel de diagnóstico criado.');
            }, 1500); // Pequeno delay para garantir que a página e outros painéis estejam carregados
        }
