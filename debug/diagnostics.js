...
// ================== ADICIONE ESTE CÓDIGO EM QUALQUER LUGAR DO DIAGNOSTICS.JS ==================
// Pode ser no início, meio ou fim do arquivo

(function() {
    'use strict';
    
    console.log('🛠️ INICIANDO DIAGNÓSTICO DO BOTÃO ADMIN...');
    
    // Aguardar carregamento
    setTimeout(function() {
        
        // 1. VERIFICAÇÃO RÁPIDA DO BOTÃO
        console.log('🔍 VERIFICAÇÃO RÁPIDA DO BOTÃO ADMIN');
        
        const adminBtn = document.querySelector('.admin-toggle') || 
                        document.querySelector('[onclick*="toggleAdmin"]') ||
                        document.querySelector('[onclick*="adminPanel"]');
        
        if (adminBtn) {
            console.log('✅ BOTÃO ENCONTRADO:', {
                tag: adminBtn.tagName,
                id: adminBtn.id || 'sem id',
                classes: adminBtn.className,
                onclick: adminBtn.getAttribute('onclick')
            });
            ...
            console.log('📏 TAMANHO E POSIÇÃO:', {
                width: rect.width + 'px',
                height: rect.height + 'px',
                top: rect.top + 'px',
                left: rect.left + 'px',
                visible: rect.width > 0 && rect.height > 0 ? '✅ VISÍVEL' : '❌ INVISÍVEL'
            });
            ...
            try {
                adminBtn.click();
                console.log('✅ CLIQUE SIMULADO - verifique se algo aconteceu');
            } catch (error) {
                console.error('❌ ERRO NO CLIQUE:', error.message);
            }
            ...
            console.log('🔍 PROCURANDO BOTÕES SIMILARES...');
            const allButtons = document.querySelectorAll('button');
            allButtons.forEach((btn, i) => {
                const text = btn.textContent + btn.innerHTML;
                if (text.includes('admin') || text.includes('cog') || text.includes('⚙️')) {
                    console.log(`⚠️ Botão ${i} pode ser admin:`, text.substring(0, 50));
                }
            });
        }
        
        // 2. VERIFICAR FUNÇÃO TOGGLEADMINPANEL
        console.log('\n🔧 VERIFICANDO FUNÇÃO toggleAdminPanel...');
        if (typeof toggleAdminPanel === 'function') {
            console.log('✅ FUNÇÃO DISPONÍVEL');
            console.log('📝 Código (primeiras 200 caracteres):', 
                toggleAdminPanel.toString().substring(0, 200) + '...');
        } else {
            console.error('❌ FUNÇÃO NÃO ENCONTRADA!');
            
            // Criar função básica se não existir
            console.log('🔄 CRIANDO FUNÇÃO BÁSICA...');
            window.toggleAdminPanel = function() {
                alert('🛠️ Painel Admin (função de teste)\n\nBotão está funcionando!');
                console.log('✅ Função toggleAdminPanel executada');
            };
            console.log('✅ Função criada. Use toggleAdminPanel() para testar.');
        }
        
        // 3. CRIAR TESTE SIMPLES NO PAINEL (se houver painel)
        console.log('\n🎯 CRIANDO TESTE NO PAINEL DE DIAGNÓSTICOS...');
        
        // Verificar se existe TestManager
        if (typeof TestManager !== 'undefined') {
            console.log('✅ TestManager disponível');
            
            // Criar teste simples
            const adminTest = {
                id: 'admin-button-check',
                title: '🛠️ BOTÃO ADMIN',
                description: 'Verifica se o botão admin está funcionando',
                type: 'verification',
                icon: '🛠️',
                category: 'ui',
                
                execute: function() {
                    console.group('🛠️ TESTE DO BOTÃO ADMIN');
                    
                    const btn = document.querySelector('.admin-toggle');
                    const hasFunction = typeof toggleAdminPanel === 'function';
                    
                    if (btn && hasFunction) {
                        console.log('✅ TUDO OK!');
                        console.log('- Botão encontrado');
                        console.log('- Função disponível');
                        return { status: 'success', message: 'Botão admin funcional' };
                    } else {
                        console.error('❌ PROBLEMAS:');
                        if (!btn) console.log('- Botão não encontrado');
                        if (!hasFunction) console.log('- Função não encontrada');
                        return { status: 'error', message: 'Problemas no botão admin' };
                    }
            ...
            // Registrar teste se não existir
            if (TestManager.registerTest) {
                try {
                    TestManager.registerTest(adminTest);
                    console.log('✅ Teste registrado no painel');
                } catch (e) {
                    console.log('⚠️ Não foi possível registrar o teste:', e.message);
                }
            }
            
        } else {
            console.log('⚠️ TestManager não disponível - criando botão flutuante direto');
            
            // Criar botão flutuante de diagnóstico
            const diagBtn = document.createElement('button');
            diagBtn.innerHTML = '🔧';
            diagBtn.title = 'Diagnóstico Botão Admin';
            ...
            diagBtn.onclick = function() {
                console.clear();
                console.log('🔧 DIAGNÓSTICO COMPLETO DO BOTÃO ADMIN');
                console.log('='.repeat(50));
                
                // Executar verificação novamente
                const btn = document.querySelector('.admin-toggle');
                console.log('1. Botão .admin-toggle:', btn ? '✅ ENCONTRADO' : '❌ NÃO ENCONTRADO');
                ...
                console.log('2. Função toggleAdminPanel:', 
                    typeof toggleAdminPanel === 'function' ? '✅ DISPONÍVEL' : '❌ NÃO DISPONÍVEL');
                
                console.log('3. Teste de clique:');
                if (btn && typeof toggleAdminPanel === 'function') {
            ...
            console.log('✅ Botão de diagnóstico criado (laranja, abaixo do admin)');
        }
        
        console.log('\n🎉 DIAGNÓSTICO COMPLETO!');
        console.log('💡 Comandos disponíveis:');
        console.log('   - toggleAdminPanel() - Testar função');
