...
// Comando 1: Forçar criação do botão
document.getElementById('post-validation-control')?.remove();
const btn = document.createElement('div');
btn.innerHTML = '<button style="position:fixed;bottom:100px;right:20px;z-index:999999;background:red;color:white;padding:15px;border-radius:50%;font-size:20px;cursor:pointer;" onclick="window.PV.panel()">🔍</button>';
document.body.appendChild(btn);
console.log('✅ Botão criado - Clique no 🔍 vermelho');

// Comando 2: Criar painel diretamente
window.PV = window.PV || {};
window.PV.panel = function() {
    const panel = document.querySelector('.post-validation-panel');
    if (panel) panel.remove();
    // Execute o código grande acima para ter a função completa
    console.log('Execute o código completo acima primeiro');
};
window.PV.panel();
